...
使用Kickstart安装centos时出现无法识别网卡
  的现象。具体表现为：安装时提示错误："Unable to download the kickstart file.  Please modify the kickstart 
  parameter below..."

找到具体网卡的linux驱动并下载

===***如果是使用光盘或其它方式安装系统，请按如下方式安装网卡驱动：***===
...
#insmod /lib/modules/2.6.18-128.el5/kernel/drivers/net/atl1e/atl1e.ko
#modprobe atl1e echo “alias eth0 atl1e” >> /etc/modprobe.conf
...
#我们刚才看到的是编译安装而来的，也有可能驱动在一个rpm包的，你也可以将其解开。
